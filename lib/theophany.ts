...
export async function getMemories(sessionId: string, limit = 12): Promise<TheophanyMemory[]> {
  ...
  const result = await supabaseQuery<any>(`select id,session_id,kind,content,metadata,importance,created_at,updated_at from public.theophany_memories where session_id=${sqlText(sessionId)} order by importance desc, updated_at desc limit ${safeLimit};`);
