...
function b64url(data: Buffer) { return data.toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, ''); }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const rows: any = await query('delete from public.theophany_oauth_states where state=' + sqlText(state) + ' and expires_at>now() returning session_id,provider;');
    const row = Array.isArray(rows) ? rows[0] : null;
    if (!row?.session_id || String(row.provider) !== 'tiktok') throw new Error('TikTok OAuth state is invalid or expired.');
    ...
    if (!tokenResponse.ok || !token.access_token) throw new Error(token.error_description || token.error || 'TikTok token exchange failed.');
    ...
    const metadata = sqlText(JSON.stringify({ token_type: token.token_type || 'Bearer', open_id: token.open_id }));
    await query(
      ...
      sqlText(crypto.randomUUID()) + ',' + sqlText(row.session_id) + ",'tiktok','connected'," + sqlText(access) + ',' +
      (refresh ? sqlText(refresh) : 'null') + ',' + (expiresAt ? sqlText(expiresAt) : 'null') + ',' + scopes + '::text[],' + metadata +
    ...
    return res.redirect('/?integration=tiktok&status=error&reason=' + encodeURIComponent(error?.message || 'TikTok OAuth failed.'));
  }
}
