...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (provider !== 'google-drive' && provider !== 'google-calendar') throw new Error('Unsupported Google OAuth provider.');
    ...
    await query(
      ...
      sqlText(crypto.randomUUID()) + ',' + sqlText(row.session_id) + ',' + sqlText(provider) + ",'connected'," + sqlText(access) + ',' +
